...
/*   Updated: 2024/05/13 11:09:58 by evan-ite         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/cub3d.h"

static uint32_t	create_col(int red, int green, int blue, t_map *map)
...
int	extract_color(int start, char *str, t_map *map)
{
	...
	if (!rgb)
		handle_error(ERR_MEM, 1, map, NULL);
	result = create_col(ft_atoi(rgb[0]), ft_atoi(rgb[1]), ft_atoi(rgb[2]), map);
